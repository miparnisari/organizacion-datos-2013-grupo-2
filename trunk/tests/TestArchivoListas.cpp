...
//	assert(archivo.abrir("", DIRECCION) == RES_OK);
	...
	assert(archivo.agregar(&lista1) == 0);

	id=11;
	lista1.agregar_campo((char*)&id,sizeof(int));
	assert (lista1.get_cantidad_campos() == 2);
	assert (archivo.agregar(&lista1) > 0);
