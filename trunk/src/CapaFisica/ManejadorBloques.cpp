...
/*
int ManejadorBloques::agregar_registro(char* nombreArchivo, RegistroVariable & reg)
{}*/
...
//		bloque.guardar(archivo);
	...
	while( archivo.good() && bloqueLibreHallado!= RES_OK ){
...
//		bloque.guardar(archivo);
	...
	//bloque.limpiar_buffer(); FIXME por que se llama?
	bloque.agregar_registro(dato,tamanioDato);
...
int ManejadorBloques::agregar_archivo(char* nombreArchivoDestino,char* nombreArchivoFuente)throw(){
	...
	if(tamanioArchivoFuente >= REG_VAR_MAX_TAM)
		return RES_ERROR;


	archivoFuente.seekg(0,ios::beg);

	char* buffer = new char[tamanioArchivoFuente];
	archivoFuente.read(buffer,tamanioArchivoFuente);
	RegistroVariable rv;


	if(rv.agregar_campo(buffer,tamanioArchivoFuente) == RES_OK){
...
int ManejadorBloques::listar_archivo_completo(char* nombreArchivo)throw(){
	...
	char* buffer = new char[tamanioBloque];
